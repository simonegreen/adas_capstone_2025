...
def kmedoids_clustering(selected_features, mode, n_clusters=2):
    ...
    try:
        k_options = range(2, 6)
        best_k = max(k_options, key=lambda k: gpu_silhouette_score(X_scaled, KMedoids(n_clusters=k, method='alternate', nit='k-medoids++', max_iter=1500).fit_predict(X_scaled)))
    except:
        best_k = 2
